...
class TrajectoryModel(nn.Module):
    ...
    def forward(self, path, text, path_mask, tgt):
        ...
        tgt_len = tgt.size(1)
        text_len = text['input_ids'].size(1)

        # Embedding
        emb_src = self.input_embedding(path) #+ #self.positional_encoding[:path_len].unsqueeze(0)
        emb_tgt = self.input_embedding(tgt) #+ self.positional_encoding[:tgt_len].unsqueeze(0)
        emb_text = self.text_encoder(text['input_ids'], text['attention_mask'])
        emb_src = emb_src.reshape(emb_src.shape[0], -1, emb_src.shape[-1])
        
        #print(f"e: {emb_src.shape}")
        #print(f"txt: {emb_text.shape}")
        # Combine
        emb_src = torch.cat([emb_src, emb_text], dim=1)
        #emb_src = emb_src.transpose(0, 1)
        #emb_tgt = emb_tgt.transpose(0, 1)
        # Masks
        text_mask = text['attention_mask'] == 0  # Convert to bool padding mask
